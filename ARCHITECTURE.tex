...
Precipitation/drought is \textbf{not} sped up the same way:
\texttt{calcular\_anomalias\_lluvia.py}'s \texttt{load\_grid\_data} takes
\texttt{shapefile} as a required argument with no \texttt{None} path,
clipping unconditionally inside the load -- splitting decode from clip
there would mean editing that function's contract, not just calling it
differently, and was left as a separate, not-yet-done follow-up rather
than rushed alongside everything else in this session.

This changes performance only, not the underlying computation -- the
open multiprocessing-correctness question from \Cref{sec:reproducibility}
is unaffected by this change and remains open regardless of which version
of this notebook is run.
