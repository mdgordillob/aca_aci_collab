...
| [`notebooks/aca_opt_multi_region_monthly.ipynb`](notebooks/aca_opt_multi_region_monthly.ipynb) | Goes further: temperature + wind + precipitation + drought across the 4 documented regions, sourced entirely from Drive. **§3 fetches the raw grib archive as a single cached zip instead of 195 individual file copies** (the classic slow-transfer-off-a-mounted-Drive pattern -- first run still pays the per-file cost once, then caches one archive to your own Drive so every run after restores from that single file), **and now tries the Drive API directly instead of the FUSE mount for that restore/cache step** (bypasses per-operation mount overhead on large files, authenticated as your own account, falls back to the mount-based read/write on any failure -- unverified beyond code review, no Drive access outside Colab to test it against, see `ARCHITECTURE.pdf` §9.16). Temperature/wind decode each year's raw grib **once** and clip to all 4 regions from that single decode (~4x fewer redundant grib-decodes), **and now cache that decoded-but-unclipped data to Drive per year, uploaded as each year finishes** -- a real run showed this stage taking 53.6 minutes for temperature alone with no caching; decode itself measured at 20-45s/year vs. ~1s to reload a cached year, so a re-run should skip almost all of that. Precipitation/drought parallelizes across **regions** instead of years -- drought has a real year-over-year ordering dependency that makes year-level parallelism unsafe, see `ARCHITECTURE.pdf` §9.9. **Caches the Stage 1/2 baseline to your own Drive** (§3b/§5b) so a re-run skips ~20-30 minutes instead of redoing it. Includes real validation cells (§8: compare vs. official reference, cross-region sanity checks), not just a file-exists summary. §9 renders `article1.tex` (the authoritative methodology paper) inline; §10 cross-validates the daily (`run_aci_colombia.py`) vs. monthly (OPT) pipelines against each other in unstandardized units, and **caches each year's `aci_daily/` preprocessing to Drive** the same way §3b/§5b cache the baseline -- a re-run only recomputes years not already cached instead of redoing all 35 years (~51 min) every time. §11 gives ad-hoc raw-`.grib` access outside the fixed pipeline; §12 loads this run's own output straight into `aci_lib`. §10's DOY-percentile step now runs on **CUDA via `cupy`** when a GPU runtime is selected (auto-detected, falls back to identical numpy otherwise) -- but that's the only GPU-portable piece of the whole pipeline (~21s of a run that takes tens of minutes): GRIB decoding (`cfgrib`) and shapefile clipping (GDAL) are CPU-only C libraries with no CUDA path, so a GPU runtime won't make the overall run meaningfully faster. See `ARCHITECTURE.pdf` §9.14 for the profiling behind that. §13 is different from everything above it -- it builds and runs a third-party CUDA GRIB tool (**unverified, 0-star single-contributor repo, timing benchmark only, no output usable by anything else**) purely so its claims can be checked directly on a real GPU; see `ARCHITECTURE.pdf` §9.15 before running it. Profiling a GPU question for the anomaly calculation itself found something better: `calcular_anomalias()`/`calcular_anomalias_viento()` were re-clipping the *percentile file* to the same shapefile on every single call (~90% of the function's own time) even though the clipped result never changes for a given (file, month, region) -- now cached, ~18-30x faster on repeat calls within the same Stage 3 worker, see `ARCHITECTURE.pdf` §9.17. **Stage 3's final per-region output CSVs are now also cached to Drive, keyed by year range** -- unlike every other cache here, a hit reuses a *prior run's actual result* and prints a loud warning that the open multiprocessing-correctness question (§9.4) wasn't re-checked; see `ARCHITECTURE.pdf` §9.18 before trusting one. |
